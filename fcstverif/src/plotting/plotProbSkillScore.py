...
def plot_rpss_map(var, yyyymm, region_name, fig_dir):
    ...
    if not os.path.isfile(rpss_file):
        logger.warning(f"[RPSS] file not exist: {rpss_file}")
        return

    with xr.open_dataset(rpss_file) as ds:

        varname = f"{var}_rpss"
        if varname not in ds:
            logger.warning(f"[RPSS] variable not found: {varname}")
            return
...
def plot_roc_by_lead_per_init(var, yyyymm, region_name, data_dir, fig_dir):
    ...
    with xr.open_dataset(auc_file) as ds_auc:
        ds_auc = ds_auc[f"{var}_auc"].squeeze()  # (time, category)
