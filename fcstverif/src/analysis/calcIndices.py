...
def plot_index_plum_by_init(fcst, obs, idx, yyyymm, fig_dir):
    ...
    fig, ax = plt.subplots()
    ftime = pd.to_datetime(fcst.time.values)
    otime = pd.to_datetime(obs.time.values)
    logger.debug(ftime)
    logger.debug(otime)

    for ens in range(len(fcst.ens)):
        ax.plot(ftime, fcst.isel(ens=ens).values, color='indianred', linestyle='--', linewidth=.5, alpha=.7)
    ax.plot(ftime, fcst.mean("ens").values, 'ro-') # ensemble mean
    ax.plot(otime, obs.values, 'ko-') # OBS
    ...
    ax.set_xticks(ftime)
    ax.set_xticklabels([t.strftime('%Y-%m') for t in ftime])
...
def calculate_index(var, yyyymm_list, model, fcst_dir, obs_dir, idx_dir, fig_dir, mode='ALL'):
    ...
    for yyyymm in yyyymm_list:
            ...
            with xr.open_dataset(fcst_file) as ds_fcst:
                ...
                if mode in ['ENSO', 'ALL']:
                    ...
                    #enso_index_fcst_sel = enso_index_fcst.isel(time=fc_idx).assign_coords(time=("time", common_time))
    ...
    # time series trajectory of all init
    if mode == "ENSO":
