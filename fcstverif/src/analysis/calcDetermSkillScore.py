...
def _clip_inputs(region: str, fcst: xr.DataArray, obs: xr.DataArray) -> tuple:
    """
...
def calc_rmse_vec(region: str, fcst: xr.DataArray, obs: xr.DataArray) -> xr.DataArray:
    """
    ...
    """
    fcst_clip, obs_clip = _clip_inputs(region, fcst, obs)
    return np.sqrt(((fcst_clip - obs_clip)**2).mean(("lat","lon")))

def calc_acc_vec(region: str, fcst: xr.DataArray, obs: xr.DataArray) -> xr.DataArray:
    """
...
def compute_deterministic_scores(
    ...
    for yyyymm in yyyymm_list:
            ...
            acc  = calc_acc_vec(region_name, fcst_da, obs_sub)       # (ens, time)
            rmse = calc_rmse_vec(region_name, fcst_da, obs_sub)     # (ens, time)

            # calculate skill score for ensemble mean
            acc_mean = calc_acc_vec(region_name, fcst_da.mean("ens"), obs_sub)  # (ens, time)
            rmse_mean = calc_rmse_vec(region_name, fcst_da.mean("ens"), obs_sub) 
