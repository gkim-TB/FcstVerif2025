...
def make_onehot(
    ...
    if transpose_dims:
        # 안전하게 transpose: 요청된 차원이 실제 존재하는지 확인
        try:
            # 일부 경우 차원 이름이 다를 수 있으므로 KeyError/ValueError를 잡는다
            onehot_da = onehot_da.transpose(*transpose_dims)
        except (ValueError, KeyError) as e:
            logger.debug(f"make_onehot: transpose failed with {e}, returning non-transposed DA")
    return onehot_da
...
def categorize_obs_tercile(var, years, obs_dir):
    ...
    try:
        ...
        logger.exception(f"Failed to open/read std file {std_file}: {e}")
        return
    ...
    for year in years:
        ...
        try:
            ...
            logger.exception(f"Failed OBS categorization for year={year}, var={var}: {e}")
            continue
...
def categorize_fcst_tercile_det(var, yyyymm, fcst_dir, stat_dir, out_dir):
    ...
    logger.info(f"[CATE DET] FCST categorize: yyyymm={yyyymm}, var={var}")
    ...
    try:
        ...
        logger.exception(f"Failed FCST deterministic categorization for yyyymm={yyyymm}, var={var}: {e}")
        return None
...
def categorize_fcst_tercile_prob(var, yyyymm, fcst_dir, stat_dir, out_dir):
    ...
    logger.info(f"[CATE PROB] FCST categorize: yyyymm={yyyymm}, var={var}")
    ...
    try:
        ...
        logger.exception(f"Failed FCST probabilistic categorization for yyyymm={yyyymm}, var={var}: {e}")
        return None