...
def normalize_time_index(time_obj, coerce=True, nan_frac_warn=0.5):
    vals = getattr(time_obj, "values", time_obj)
    vals = np.asarray(vals)
    try:
        dt = pd.to_datetime(vals, errors="coerce" if coerce else "raise")
    except Exception:
        dt = pd.to_datetime([str(x) for x in vals], errors="coerce")
    dt = pd.DatetimeIndex(dt)
    if nan_frac_warn is not None and len(dt) > 0:
        frac = pd.isna(dt).sum() / len(dt)
        if frac > nan_frac_warn:
            logger.warning("normalize_time_index: high NaT fraction %.2f sample=%s", frac, dt[:6])
    return dt
