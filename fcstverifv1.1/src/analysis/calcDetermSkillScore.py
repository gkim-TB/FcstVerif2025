...
from fcstverif.src.utils.general_utils import load_obs_data, clip_to_region, get_combined_mask, match_common_times_by_month, normalize_time_index
...
def compute_deterministic_scores(
    ...
    for yyyymm in yyyymm_list:
            ...
            with xr.open_dataset(fcst_file) as ds_fcst:
                fcst_time = normalize_time_index(ds_fcst['time']) # (lead,) datetime64
                ...
                logger.debug("fcst_time type=%s dtype=%s sample=%s",
                        type(ds_fcst['time']), ds_fcst['time'].dtype, ds_fcst['time'].values[:5])
