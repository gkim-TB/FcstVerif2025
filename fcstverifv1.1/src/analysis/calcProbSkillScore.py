...
from fcstverif.src.utils.general_utils import load_obs_data, clip_to_region, match_common_times_by_month, normalize_time_index
...
def compute_roc_auc_all_categories(var, fcst_prob, obs_ohe, init_time, region_name):
    ...
    logger.debug(fcst_sub)
    logger.debug(obs_sub)
...
def compute_probabilistic_scores(
    ...
    for yyyymm in yyyymm_list:
        ...
        with xr.open_dataset(prob_file) as ds_prob:
            ...
            fcst_time = normalize_time_index(ds_prob['time']) # (lead,) datetime64
