...
def compute_anomaly(
    ...
    파일 이름 예:
      ...
      - forecast: {forecast_dir}/ensMean_{var}_{YYYYMM}.nc
    """
    ...
    for date in dates:
        ...
        fcst_file = os.path.join(forecast_dir, f"ensMean_{var}_{yyyymm_str}.nc")
        if var == 'sst':
            hind_file = os.path.join(hindcast_dir, f"ens_{var}_{yyyymm_str}.nc")
            fcst_file = os.path.join(forecast_dir, f"ens_{var}_{yyyymm_str}.nc")


        # 두 파일이 모두 존재해야 anomaly를 계산
        if not os.path.isfile(hind_file):
            print(f"[WARNING] Hindcast file not found: {hind_file}")
            continue
        if not os.path.isfile(fcst_file):
            print(f"[WARNING] Forecast file not found: {fcst_file}")
            continue
        ...
        out_fname = os.path.join(out_dir, f"{var}_anom_{yyyymm_str}.nc")
        ds_anom.to_netcdf(out_fname)
        print(f"[INFO] Saved anomaly: {out_fname}")

    print("Done. Anomaly files are in:", out_dir)
