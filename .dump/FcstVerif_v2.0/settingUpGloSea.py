...
def open_kma_fcst_to_climpred_format(grib_file, init_date_str, var):
    """
    glos_conv_kma_fcst_6mon_mon_t15m_YYYYMMDD.grb2 파일을 열어서
...
def make_monthly_ensemble_forecast(
    ...
    print(f"\n[INFO] Processing var={var}, prefix={file_prefix}")
    print("weekly_mondays:", weekly_mondays)
    print("last_mondays:", last_mondays)
    ...
    for wmonday in weekly_mondays:
        ...
        if not os.path.isfile(grib_file_path):
            print(f"[WARNING] File not found: {grib_file_path}")
            continue
        ...
        if wmonday in last_mondays:
            ...
            if not monthly_ens_list:
                print(f"[WARNING] No data accumulated for month={wmonday.month}")
                continue
