...
def compute_regional_scores(var, years, fcst_dir, obs_dir, out_dir, region_name, region):
    ...
    for yy in years:
        for mm in range(1, 13):
            ...
            fcst_da = ds_fcst[var].squeeze()  # (lead, lat, lon)
            ...
            for lead_idx in range(fcst_da.sizes['lead']):
                ...
                try:
                    ...
                    if (obs_sel.lat.shape != fcst_da.lat.shape) or (obs_sel.lon.shape != fcst_da.lon.shape):
                        print(f"[INFO] Interpolating obs grid to match forecast grid for {target_time}")
                    ...
                    print(f"[WARN] obs에 {target_time}이 없습니다. ==> NaN 처리합니다.")
            ...
            ds_out = xr.Dataset({
                ...
                'rmse': (['lead'], rmse_list),
                'bias': (['lead'], bias_list)
            }, coords={'lead': fcst_da['lead'].values})

            out_file = os.path.join(region_out_dir, f"score_{var}_{yyyymm}.nc")
            ds_out.to_netcdf(out_file)
            print(f"[INFO] Saved regional scores => {out_file}")
